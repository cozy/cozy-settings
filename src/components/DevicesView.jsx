...
class DevicesView extends Component {
  ...
  render () {
    const { t, f, isFetching, devices, openDeviceRevokeModale, deviceToRevoke, onDeviceModaleRevoke, onDeviceModaleRevokeClose, devicePerformRevoke } = this.props
    return (
      <div role='contentinfo' className={devicesStyles['devices-view']}>
        <h2 className={viewStyles['set-view-title']}>{t('DevicesView.title')}</h2>
        { isFetching && <Loading /> }
        { !isFetching && devices.length === 0 && <Empty
          icon={EmptyIcon}
        ...
        { !isFetching && devices.length > 0 && (
          <div className={classNames(
            ...
            <div className={classNames(tableStyles['coz-table-head'], tableStyles['coz-table-row'])}>
              ...
              <div className={classNames(tableStyles['coz-table-header'], tableStyles['set-table-date'])}>{ t('DevicesView.head_sync') }</div>
