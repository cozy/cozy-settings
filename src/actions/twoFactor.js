...
export const activate2FA = (mode = AUTH_MODE.TWO_FA_MAIL) => {
  return (dispatch, getState) => {
    ...
    cozyFetch('PUT', '/settings/instance/auth_mode',
      ...
      dispatch({ type: ACTIVATE_2FA_FAILURE, error: 'ProfileView.infos.server_error' })
    })
  }
}
...
export const desactivate2FA = () => {
  return (dispatch, getState) => {
    ...
    cozyFetch('PUT', '/settings/instance/auth_mode',
      ...
      dispatch({ type: DESACTIVATE_2FA_FAILURE, error: 'ProfileView.infos.server_error' })
    })
  }
}
...
export const checkTwoFactorCode = (code, mode = AUTH_MODE.TWO_FA_MAIL) => {
  return (dispatch, getState) => {
    ...
    if (code === '') {
      dispatch({ type: CHECK_TWO_FACTOR_CODE_FAILURE, error: 'ProfileView.infos.empty' })
      return
    }
    cozyFetch('PUT', '/settings/instance/auth_mode', {
    ...
    .catch(() => {
      dispatch({ type: CHECK_TWO_FACTOR_CODE_FAILURE, error: 'ProfileView.infos.server_error' })
    })
  }
}
